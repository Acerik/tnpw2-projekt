...
function ListAdvertises() {
    ...
    const [advertisesSort, setAdvertisesSort] = useState(Number(useParams().sort ? useParams().sort : 1));
    ...
    if (firstLoad) {
        ...
        tempConfig.params = {page: page, search: searchText, sort: advertisesSort};
    ...
    function navigateHref(nPage, nSort) {
        nSort = !nSort ? advertisesSort : nSort;
        nSort = nSort === advertisesSort ? advertisesSort : nSort;
        return ((searchText === false) ? ('/inzeraty/' + nPage + '/' + nSort) : '/inzeraty/hledat/' + searchText + '/' + nPage + '/' + nSort);
    }
    ...
    function sortAdvertises(e) {
        ...
        setAdvertisesSort(value);
        navigate(navigateHref(page, value));
        navigate(0);
    }
    ...
    return (
        <div id='content'>
            ...
            {/*Řazení inzerátů podle stáří*/}
            <Form.Select hidden={advertises.length <= 0} className="ms-auto"
                         style={{width: 200}} onChange={sortAdvertises} value={advertisesSort}>
